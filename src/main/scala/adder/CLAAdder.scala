...
class CLAAdder(n: Int, name: String = "default_adder_name")
  ...
  val turn_on_println = false
  override val desiredName = s"width${n}_" + name

  if (name.length > 96) {
    throw new IllegalArgumentException(
      "CLAAdder's name is too long, you might have composed a name that is too verbose, or your CLAAdder might be experiencing infinite recursion problems"
    )
  }

  val max_children =
    4 //how many children a CLA (not CLAAdder) block can directly derive
  ...
  n match {
    ...
    case _ => {
      ...
      val tree_depth =
        floor(
          log(n - 1) / log(max_children)
        ).toInt //e.g. floor{log(18)/log(4)} = 2
      ...
      val num_adders =
        if (leftover_adder_width > 0) 1 + num_whole_granularity_adders
        else num_whole_granularity_adders
      ...
      val whole_granularity_adders =
        (0 until num_whole_granularity_adders).toList.map { case x =>
          Module(new CLAAdder(granularity, name + "." + x.toString))
        }

      val sum_collector = Wire(
        Vec(num_whole_granularity_adders, UInt(granularity.W))
      )

      //connect the whole-granularity modules
      (0 until num_whole_granularity_adders)
        .map(_ * granularity)
        .zip(whole_granularity_adders)
      ...
      //connect the leftover part
      if (leftover_adder_width > 0) {
        val leftover_adder = Module(
          new CLAAdder(
            leftover_adder_width,
            name + "." + num_whole_granularity_adders.toString
          )
        ) //the leftover adder will map to the highest-index bits
